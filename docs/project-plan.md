...
This is the original architecture/build-plan document — the section numbers (`§1`–`§14`) referenced throughout this repo's code comments, `BACKLOG.md`, and `CONTRIBUTING.md` point here. For a quick project overview and local setup, see the root [README.md](../README.md).
...
**Principle: leverage existing content over manual entry.** A neighborhood or business already publishes events, photos, and updates somewhere else (an events-calendar plugin's iCal/webcal feed, an Instagram feed, etc.) — pulling that in automatically means zero ongoing effort from the neighborhood admin or business owner, and the app feels current/alive without anyone having to re-key anything into Blockwise. Manual entry (announcements authored directly, events added by hand) is the fallback for whoever doesn't have an external source, not the primary path. This is also the monetization thesis (§11): revenue comes from creating/maintaining neighborhoods and from businesses' coupons/announcements delivering in real time, so the less friction there is in keeping that content fresh, the stronger the paid product is.
...
- Social links (Instagram, shipped v0.18.0) and iCal/webcal event feed import (backlog Ref 30) are the first two "leverage existing content" sources; both feed the neighborhood/business "what's happening now" surface (backlog Ref 27) alongside manually authored announcements.
