...
Last reviewed: 2026-07-13 (Neighborhood-admin dashboard redesigned as a standalone sidebar shell — see History).
...
```text
apps/web/src/app/
├── layout.tsx                                    (root layout — SiteChrome swaps in AccountNav/Footer, or hides them entirely for the neighborhood-admin sidebar shell)
├── SiteChrome.tsx                                 (client component, no route — hides AccountNav/Footer on /neighborhood-admin/:slug/* routes, which supply their own sidebar shell chrome)
...
│   └── [neighborhoodSlug]/
│       ├── layout.tsx                              — S (neighborhoodAdminGate on every tab's data calls) — standalone sidebar shell (Ref 31; SiteChrome hides the site's AccountNav/Footer here), resolves slug→id, sidebar nav with location/pending-claim counts
│       ├── page.tsx                                /neighborhood-admin/:slug — Overview tab (stat tiles, description, social links, events, read-only active POIs)
