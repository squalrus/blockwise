...
If your change touches Geoapify Places data, re-read [project plan](./docs/project-plan.md) §1.1 first — its field-mask billing model was written for Google Places but the same discipline (request only what a caller actually needs, respect the daily credit pool) carries over to Geoapify's per-endpoint credit weights (`apps/api/src/places/quotaGuard.ts`), not by convention. Don't bypass it to "simplify" a data path. (Yelp Fusion integration was dropped from the plan — see [BACKLOG.md](./BACKLOG.md) — but if it's ever picked back up, re-read §1.1 for its stricter 24-hour content TTL before touching that data path.)
