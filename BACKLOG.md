...
- [x] Phase 3 — Schema changes (rename `venue.google_place_id` → `geoapify_place_id`, trim `venue_enrichment_cache`). `venue_enrichment_cache` drops `rating`/`reviews`/`photo_refs`/`price_tier`/`atmosphere` (`supabase/migrations/20260828020000_geoapify_venue_schema.sql`); ratings/reviews/photo galleries removed as product features across the frontend (rating stat tile, About tab photo strip, Reviews tab, `aggregateRating` JSON-LD, photo-based OG image) and the now-dead `GET /locations/:id/photo` proxy route deleted. `price_tier`/`atmosphere` dropped outright rather than kept-but-unpopulated (no Geoapify equivalent confirmed, no reason to carry the columns forward speculatively). **Accepted temporary regression** (matching Phase 2's pattern): `sync.ts`/`review.ts`/`investigate.ts`/`refresh.ts` still call Google's live API, so `geoapify_place_id` holds a real Google place ID and `venue_enrichment_cache.source = 'geoapify'` fronts data that's still actually Google-sourced, until Phase 4 rewires the real client.
- [x] Phase 4 — `sync.ts`/`geo.ts` wiring (depends on 1–3). Full cutover, not a hybrid: `apps/api/src/places/client.ts` (Google) and `mockClient.ts` deleted outright; `instrumentedClient.ts`/`quotaGuard.ts`/`sync.ts`/`preview.ts`/`locations/review.ts`/`enrichment/refresh.ts`/`scripts/syncPlaces.ts`/app.ts's `getPlacesClient()` composition root all rewired onto `geoapifyClient.ts`'s `LiveGeoapifyClient`/`MockGeoapifyClient`, keyed by a new `GEOAPIFY_API_KEY` env var (`GOOGLE_PLACES_API_KEY` no longer read anywhere). `PLACES_API_RESULT_CAP` raised from Google's fixed 20 to Geoapify's documented default-adjustable `limit` (used at 500, its documented max) -- real-world saturation behavior at this cap is still unverified (Phase 0's live tiled-sweep ceiling item), a documented assumption not a load-tested one. `searchPlacesInPolygon` now sends every taxonomy-configured Geoapify tag as the `categories` filter (deduped) since Geoapify's Places API requires at least one non-empty category with no wildcard, unlike Google's old "omit includedTypes for unrestricted" -- this also fixes the categorization gap Phases 2/3 called out (places now actually match the taxonomy). `businessStatus`/`skippedClosedPermanently` is a permanent no-op (field kept at 0 for API/report shape compatibility) -- no OSM equivalent exists. **Extra scope beyond the plan doc's own phase assignment, done now instead of Phase 8:** `investigate.ts` moved onto a new `GeoapifyTextSearchClient.searchText` (Geoapify's Geocoding API, `v1/geocode/search`) rather than staying on Google -- the plan doc's design section had filed this under item 8/Phase 8 since Geoapify had no text-search equivalent in Phase 1's scope; one was built now so `client.ts` could be deleted outright in this pass rather than kept alive for one route. Also fixed a plan-doc error along the way: hours parsing was asserted as "not affected" by the migration, but Geoapify's raw OSM `opening_hours` string (e.g. `"Mo-Fr 09:00-18:00"`) doesn't match what `locations/hours.ts`'s parser expects (Google's one-line-per-weekday format) -- added `apps/api/src/enrichment/openingHours.ts`, a compatibility parser for the common day-range + time-range case (falls back to "hours unknown" for a day it can't parse, e.g. holiday exceptions/comments), so the "open now" feature keeps working. `places_api_call_log.endpoint`'s CHECK constraint widened (migration `20260828030000_places_api_call_log_geoapify_endpoints.sql`) to add `searchPlaces` alongside the historical Google-only values, and `PlacesApiEndpoint`/`PLACES_API_PRICING` (`packages/types`) got the same minimal addition (a placeholder rate, not a real Geoapify cost model) -- full endpoint/cost-model rework stays Phase 7's job, this is just enough to not break Phase 4. `PlacesInvestigationCandidate`'s `business_status`/`types` fields removed/renamed (`categories`) to match, with the admin investigate UI's closed-business badge removed accordingly.
- [x] Phase 5 — Backfill existing venues via `locations/review.ts`'s admin diff flow (depends on 4) — shipped v0.84.5, a standalone super-admin page (`/admin/super/geoapify-migration`) rather than folded into the permanent neighborhood-admin Locations/Review UI, since it's disposable and gets deleted once every location has a real Geoapify ID. Two steps: (1) a fresh boundary search per neighborhood surfaces fuzzy name/location matches (`review.ts`'s new `PossibleLocationMatch`, `dedup.ts`'s `nameSimilarity`) against existing venues for explicit admin approval — never auto-applied, since a wrong fuzzy match would silently attach the wrong place's data; (2) whatever's left with a Google-shaped ID (`legacyPlaceId.ts`'s `isLegacyGooglePlaceId`, matching the `ChIJ` prefix) after that — either Geoapify's boundary sweep never resurfaced it (a real, confirmed coverage gap per Phase 0's live verification) or it didn't clear the fuzzy-match threshold — gets manual search-and-attach via the same free-text Geoapify lookup `investigate.ts` uses. No schema changes; reuses the existing `geoapify_place_id` column and `reassignLocationPlaceIdForNeighborhood`. **Post-ship finding (2026-09-02):** live-verified that Geoapify's own place IDs aren't as stable as assumed — an OSM name-tag edit alone ("Kipos Greek Taverna" → "Kipos") produces a new id, and the *old* id had since been reassigned by Geoapify to an unrelated Chapel Hill, NC restaurant, so the tool's own free-text search+attach step (no distance bound at all, unlike the fuzzy possible-match step's 30m) had silently attached that wrong, distant id to the real Phinneywood venue. Added a distance guardrail (`distance_meters` on every search/reverse-geocode candidate, confirm-to-attach past 500m) and a coordinate-based reverse-geocode suggestion (catches a renamed business a name search can't connect) to both this tool and — since the need for reassignment doesn't end with the migration — a new small permanent "Reassign place ID" action on the regular Locations tab (`ReassignPlaceIdPanel.tsx`), meant to survive this disposable tool's eventual teardown. Also reset all 142 previously-reconciled locations back to a legacy-shaped placeholder id, since Phase 4 shipped before any existing row could have gotten a real id from a legitimate boundary sync — every one was reconciliation-tool provenance and none could be trusted without an individual audit.
- [x] Phase 6 — Map rendering: MapLibre GL JS + Geoapify vector tiles, replacing Google Maps JS SDK. Shipped v0.84.7. `MapView.tsx` (venue map, clustering + category-colored markers + popups) and `BoundaryMap.tsx` (boundary polygon drawing) both rewritten against `react-map-gl`'s MapLibre bindings, with `@mapbox/mapbox-gl-draw` for polygon editing. Two bugs caught only by actually running it in a browser, not by typecheck/build: (1) `useControl`'s 2-callback overload treats a lone second argument as `onRemove` not `onAdd`, and React Strict Mode's dev-only double-invocation of `useMemo` factories means a naive `new MapboxDraw()` per call can leave the control pointing at a discarded instance — fixed via the explicit 3-arg `useControl` form plus an idempotent factory guard; (2) MapLibre resolves its worker script relative to its own bundled chunk URL by default, which 404s under Turbopack, and even a corrected asset URL doesn't work since the worker's own internal relative import to a sibling file breaks too — silently: no console error, the map's `load` event just never fires and nothing beyond the base style paints. Fixed by vendoring both files into `public/` verbatim via a new `predev`/`prebuild` script (`apps/web/scripts/vendor-maplibre-worker.mjs`). Also found mapbox-gl-draw's bundled default styles fail MapLibre's stricter expression validator (`line-dasharray` needs `["literal", ...]` wrapping) — patched that one property rather than hand-maintaining a full style copy.
- [x] Phase 7 — Monitoring: Geoapify-shaped `places_api_call_log`/credit tracking. `PlacesApiEndpoint` (`packages/types`) drops the Google-only `searchNearby`/`fetchPhotoMedia` values entirely (down to the 4 real Geoapify endpoints); migration `20260902010000_geoapify_credit_metering.sql` purges their historical `places_api_call_log` rows and tightens the CHECK constraint to match, since those rows described Google calls a Geoapify credit model was never going to price correctly anyway. Bigger change: Google billed per-request ($/1000 calls, a separate free tier per SKU), which no longer describes how Geoapify actually bills — Geoapify sells fixed monthly plans sized to a *daily* credit ceiling (Free = 3,000/day/$0, `docs/location-services-comparison.md`), shared across every endpoint rather than metered per-request. `PLACES_API_PRICING`'s monthly-$/free-events shape is replaced by `PLACES_API_CREDIT_COST` (a per-endpoint credits-per-request weight, all 1 today — Geoapify's "+1 credit per extra 20 results" bonus isn't modeled, since `places_api_call_log` doesn't record a per-call result count and real call volumes run well under 20 results each) and `GEOAPIFY_FREE_DAILY_CREDITS = 3000`. `PlacesApiQuotaGuard` (`apps/api/src/places/quotaGuard.ts`) now checks total weighted credits across *every* endpoint against that one shared pool each day, not `getPlaceDetails`' own count against its own tier — Google's old per-SKU guard would have missed the real risk of the shared pool being exhausted by other (admin-triggered) endpoints. `google_places_billing_month_start()`/`get_places_api_month_to_date_count` (Pacific-Time monthly boundary) are replaced by `geoapify_billing_day_start()`/`get_places_api_day_to_date_counts()` (UTC daily boundary — Geoapify's docs don't state a reset timezone, so this is a documented assumption, not a confirmed one, unlike Google's). Monitoring UI (`apps/web/.../monitoring/`): the Google Places sub-nav tab, page headers, and section copy are renamed to Geoapify throughout; the dollar-cost tiles/chart (`placesApiCost.ts`, `PlacesApiCostChart.tsx`) are replaced with credit counts (`placesApiCredits.ts`, `PlacesApiCreditsChart.tsx`) — dropped the $ estimate entirely rather than fabricating one, since Geoapify's plan-tier billing isn't a smooth per-request rate; the free-tier widget is one shared "daily credits" gauge against the 3,000 pool instead of a separate progress bar per endpoint (which would have wrongly implied each endpoint gets its own 3,000/day).
- [x] Phase 8 — Cleanup/removal. Deleted `docs/google-places-setup.md` (obsolete now that Phase 4 already deleted the Google client itself and no `GOOGLE_PLACES_API_KEY`/`GOOGLE_MAPS_API_KEY` remain anywhere in code or env files) and fixed its now-dead references in README.md/CONTRIBUTING.md. Deleted the rest of the disposable Phase 5 backfill tooling now that every location has been reconciled onto a real Geoapify place ID: `apps/api/src/places/legacyPlaceId.ts` (+ test), the `/admin/geoapify-migration/...` routes and their `GeoapifyMigration*` request/response types in `packages/types`, and the `/admin/super/geoapify-migration` page/tab. The two Geoapify candidate/reverse-geocode shapes that page shared with the permanent Reassign Place ID action (`ReassignPlaceIdPanel.tsx`, split out ahead of this teardown per the 2026-09-02 backfill finding above) were kept, renamed off the "Migration" name to `GeoapifyPlaceCandidate`/`GeoapifyReverseGeocodeResult` since they're no longer migration-specific. `deleteLocationForNeighborhood`'s `allowBusinessKind` option (added for the migration tool's own junk-row cleanup) had no other caller once that tool's DELETE route was gone, so it's removed too -- the function always blocks a business-kind delete again, matching the regular neighborhood-admin Locations tab's behavior. Also swept remaining "Google Places" copy/comments in admin UI (investigate/review/reports pages, AdminShell, boundary page) and `docs/investigating-missing-venues.md`/`docs/location-services-comparison.md` to describe current Geoapify/OSM behavior — including removing the stale "permanently closed" filtering claim, since `businessStatus` has no OSM equivalent and was already dropped from the investigate response in Phase 4. **Deliberately left for Phase 9**: `PoweredByGoogle` (still rendered on venue About pages for what's now Geoapify-sourced data) stays until Phase 9's real "Powered by Geoapify" + OpenStreetMap attribution UI replaces it, rather than patching it twice.
