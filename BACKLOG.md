...
| 106 | [Fix react-hooks strict-mode violations](#fix-react-hooks-strict-mode-violations) | improvement | S | L | — |
...
#### Fix react-hooks strict-mode violations

**Ref:** 106
**Type:** improvement
**Depends:** —
**Why** — Setting up CI/CD (Ref 25) turned on `lint` as a required gate, surfacing 10 pre-existing `react-hooks/set-state-in-effect` / `react-hooks/refs` / `react-hooks/purity` errors (part of the stricter React Compiler-era rules eslint-config-next 16.3.1 ships) across 7 files in `apps/web`. Unlike the unescaped-entity/`<a>`-vs-`<Link>` errors fixed in the same pass, these are behavioral, not mechanical — each needs the component's actual render/effect logic reviewed to fix correctly, so they were downgraded to `warn` in `apps/web/eslint.config.mjs` rather than rushed, to get CI shipped without either blocking on unrelated work or silently losing visibility.
**Notes:** Violations as of the CI rollout: `InstallPrompt.tsx:44`, `SlideTrack.tsx:74` (set-state-in-effect) and `:134` (refs — accessing a ref during render), `ThemeToggle.tsx:19`, `BoundaryMap.tsx:119`, `NearestVenues.tsx:56`, `MapView.tsx:66` (all set-state-in-effect — the general fix is deriving the value during render or moving the update into an event handler/callback instead of an effect body, per the rule's own guidance: <https://react.dev/learn/you-might-not-need-an-effect>), and `admin/business/[venueId]/coupons/page.tsx:88`, `admin/business/[venueId]/events/page.tsx:126`, `admin/neighborhood/[neighborhoodSlug]/events/page.tsx:131` (purity — `Date.now()` called during render; likely needs the "now" value computed in an effect/state or passed in rather than read live on every render). Once all 10 are fixed, remove the `rules` override block added to `apps/web/eslint.config.mjs` (Ref 25) so these three rules go back to eslint-config-next's default `error` severity.
