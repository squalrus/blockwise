...
describe("addFavorite", () => {
  it("returns not_found for an unknown venue", async () => {
    ...
    const result = await addFavorite("missing-venue", "user-1", repo);
  ...
  it("is idempotent on repeat calls for the same user", async () => {
    ...
    const first = await addFavorite("venue-1", "user-1", repo);
    expect(first.status).toBe("created");

    const second = await addFavorite("venue-1", "user-1", repo);
...
describe("removeFavorite", () => {
  it("returns not_found for an unknown venue", async () => {
    const repo = new FakeFavoriteRepository(new Set());
    const result = await removeFavorite("missing-venue", "user-1", repo);
    expect(result).toEqual({ status: "not_found" });
  });

  it("removes an existing favorite", async () => {
    const repo = new FakeFavoriteRepository(new Set(["venue-1"]));
    await addFavorite("venue-1", "user-1", repo);

    const result = await removeFavorite("venue-1", "user-1", repo);
...
describe("getFavoriteStatus", () => {
  it("returns not_found for an unknown venue", async () => {
    const repo = new FakeFavoriteRepository(new Set());
    const result = await getFavoriteStatus("missing-venue", "user-1", repo);
    expect(result).toEqual({ status: "not_found" });
  });
  ...
  it("reports false when not favorited, true after favoriting", async () => {
    ...
    const before = await getFavoriteStatus("venue-1", "user-1", repo);
    expect(before).toEqual({ status: "found", favorited: false });

    await addFavorite("venue-1", "user-1", repo);

    const after = await getFavoriteStatus("venue-1", "user-1", repo);
