...
export function ChallengesView({ neighborhoodSlug }: { neighborhoodSlug: string }) {
  ...
  // In-progress (most percent-complete first) above not-yet-started above
  // completed -- unlike the account page's own Challenges tab, not-started
  // challenges aren't excluded here, since this view is browsing what's
  // available in the neighborhood rather than a personal roll-up of
  // engaged-with challenges. Not-started ranks above completed since it's
  // the more actionable group -- something to go start -- while completed is
  // done and mainly there for the record.
  ...
  const groupCount = [inProgress, notStarted, completed].filter((g) => g.length > 0).length;
