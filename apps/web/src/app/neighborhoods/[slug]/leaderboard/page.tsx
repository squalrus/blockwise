    ...
    <ol className="flex flex-col gap-2">
      {leaderboard.map((entry) => {
        ...
        return (
          ...
          >
            <span className={`w-6 font-heading text-base font-extrabold ${isTop ? "text-brand-amber" : "text-muted"}`}>
              #{entry.rank}
            </span>
            <span className={`flex-1 font-extrabold ${isTop ? "text-nav-foreground" : "text-muted"}`}>
              {entry.display_name ?? entry.username ?? "Neighbor"}
            </span>
            <span className={`font-heading font-extrabold ${isTop ? "text-brand-amber" : "text-muted"}`}>
              {entry.points} pts
            </span>
          </li>
        );
