...
import type { NeighborhoodAnalytics } from "@blockwise/types";
...
import { LocationsByCategoryChart } from "./LocationsByCategoryChart";
import { TopVenuesLeaderboard } from "./TopVenuesLeaderboard";

type State =
  | { status: "loading" }
  | { status: "ready"; analytics: NeighborhoodAnalytics }
...
// neighborhood, backed by a single get_neighborhood_analytics RPC (one
// fetch covers check-ins-over-time, activity-by-type, locations-by-
// category-group, and a top-venues leaderboard). Loading/forbidden state
// lives in layout.tsx, mirroring every other tab.
export default function NeighborhoodAdminAnalyticsPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const res = await fetch(
        clientApiUrl(`/neighborhood-admin/neighborhoods/${neighborhoodId}/analytics?days=${days}`),
        { headers: { Authorization: `Bearer ${token}` } }
      );
      if (cancelled) return;
      if (!res.ok) {
        setState({ status: "error", message: "Failed to load neighborhood analytics" });
        return;
      }
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-[15px] text-body-text">
            Charts and breakdowns of locations and activity in the neighborhood.
          </p>
        </div>
      ...
      {state.status === "ready" && (
        <div className="grid grid-cols-1 items-start gap-5 lg:grid-cols-2">
          ...
          <section className="rounded-3xl border border-border bg-card p-6">
            <h2 className="mb-3.5 font-heading text-lg font-extrabold">Locations by category</h2>
            <LocationsByCategoryChart data={state.analytics.locations_by_category_group} />
          </section>

          <section className="rounded-3xl border border-border bg-card p-6">
            <h2 className="mb-3.5 font-heading text-lg font-extrabold">Top venues by check-ins</h2>
            <TopVenuesLeaderboard venues={state.analytics.top_venues} />
          </section>
        </div>
      )}
    </div>
