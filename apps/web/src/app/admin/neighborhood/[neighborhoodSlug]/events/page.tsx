...
import type { Event, NeighborhoodDashboardSummary } from "@blockwise/types";
...
// Events tab (BACKLOG.md Ref 78, imported Claude Design mockup "Spored
// Admin" -- Calendar feeds + Create event on the left, Upcoming list on the
// right). Split out of the Overview tab, which used to hold this same
// IcalFeedForm/EventForm/events-list block directly -- Overview now only
// carries description/social links.
export default function NeighborhoodEventsPage() {
  ...
  async function handleDeleteEvent(eventId: string) {
    const token = await getAccessToken();
    const res = await fetch(
      clientApiUrl(`/neighborhood-admin/neighborhoods/${neighborhoodId}/events/${eventId}`),
