-- "Mayor" badges: ranked #1 by 60-day check-in count ("Top Caps") at a
...
from (values
  ('business_mayor', 'Business Mayor', 'Ranked #1 for check-ins at a business in the last 60 days.'),
  ('poi_mayor', 'Landmark Mayor', 'Ranked #1 for check-ins at a point of interest in the last 60 days.'),
  ('neighborhood_mayor', 'Neighborhood Mayor', 'Ranked #1 for check-ins across a whole neighborhood in the last 60 days.')
...
from (values
  ('business_mayor', 'business_rank_reached'),
  ('poi_mayor', 'poi_rank_reached'),
  ('neighborhood_mayor', 'neighborhood_rank_reached')
) as r(code, rule_type)
