-- v14: adds request_volume_by_day_and_scope, the request_log counterpart to
-- places_api_calls_by_day_and_endpoint -- backs the Performance page's "API
-- request volume" chart's per-route-scope breakdown (App/Admin/Auth
-- overlaid, plus a total) instead of one flat daily line. Deliberately
-- ignores p_route_scope (unlike every other request_log block below, which
-- narrows to the page's selected scope) -- narrowing this breakdown to
-- whichever single scope is already selected would collapse it back to one
-- flat line, defeating the point of overlaying all three.
