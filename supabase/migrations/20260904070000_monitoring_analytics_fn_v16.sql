-- v16: adds checkin_timing_over_time, backing the new "Check-in timing" chart
-- on the Performance tab (checkin_timing_log, previous migration). Averages
-- are computed only over outcome = 'created' rows -- a too_far/cooldown/
-- not_found attempt never reaches the reward/notify/collection phases, so
-- mixing it in would silently drag those phase averages down without saying
-- so. total_avg_ms is still every attempt's wall-clock time regardless of
-- outcome, matching latency_over_time's own request_log-wide average, so the
-- two stay comparable.
