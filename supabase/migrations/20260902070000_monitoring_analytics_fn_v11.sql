-- v11: adds p_source, so the Monitoring > Errors tab's "Errors by source"
-- tiles can double as a click-to-filter (mirroring p_status_class's existing
-- "Status codes tiles filter Recent requests" pattern) -- narrows
-- errors_over_time and recent_errors to one of api/web/marketing.
-- errors_by_source itself stays unfiltered by p_source, same as
-- status_code_breakdown stays unfiltered by p_status_class: it's the tile
-- data those filters are chosen from, not something they narrow.
--
-- CREATE OR REPLACE can't add a new parameter in the middle of an existing
-- signature and have overload resolution stay unambiguous with defaults, so
-- the old 4-arg signature is dropped first (same reasoning as v10's own
-- p_days -> p_minutes cutover).
